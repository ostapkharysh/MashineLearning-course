...
class CartPoleQLearningAgent:
    def __init__(self,
        ...
        self.bin_states =[
            # Cart: (Position, Velocity) and Pole Angle, Velocity at Tip
            np.linspace(-2.4, 2.4, discret_bins + 1),       #   -2.4   2.4
            np.linspace(-5.0, 5.0, discret_bins + 1),       #   -inf   inf
            np.linspace(-0.5, 0.5, discret_bins + 1),       #   -41.8  41.8
            np.linspace(-1.5, 1.5, discret_bins + 1),       #   inf    inf
        ]

        self.max_bin = max([len(el) for el in self.bin_states])
        q_states = math.pow(self.max_bin+1, len(self.bin_states))

        self.q = np.zeros(shape=(int(q_states), self._num_actions))
    ...
    def _build_state(self, observation):
        ...
        summa = 0
        for indx, el in enumerate(observation):
            summa += (np.digitize(el, self.bin_states[indx]) * ((self.max_bin + 1) ** indx))
    ...
    def act(self, observation, reward):
        ...
        self.q[self.state, self.action] += self.learning_rate * (reward +
            self.discount_factor * max(self.q[next_state, :]) - self.q[self.state, self.action])
        # ====================================================
